...
- Ohne konfigurierten SMTP-Host antwortet der Endpoint 503.
"""
...
async def mail_status():
    """Ob Mailversand verfügbar ist und wie groß Anhänge insgesamt sein dürfen."""
    return {
        "available": bool(settings.smtp_host),
        "max_attachment_bytes": settings.mail_max_attachment_mb * 1024 * 1024,
    }
...
async def send_results(
    ...
    if not settings.smtp_host:
        raise HTTPException(status_code=503, detail="Mailversand ist auf diesem Server nicht konfiguriert")
    ...
    for file in files:
        ...
        attachments.append((file.filename or "datei.pdf", content))
