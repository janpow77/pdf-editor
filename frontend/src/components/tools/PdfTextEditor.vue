      ...
      <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr),24rem]">
            ...
            <template #overlay="{ width, height, scale }">
              ...
              <template v-if="active">
                <div class="absolute bg-white shadow-apple ring-2 ring-primary-500" :style="boxStyle(active.box, width, height)">
                  ...
                  <button
                    type="button"
                    class="absolute -left-3 -top-3 grid h-7 w-7 touch-none place-items-center rounded-full bg-primary-600 text-xs text-white shadow-sm"
                    aria-label="Textblock verschieben"
                    @pointerdown.prevent="startPointerDrag('move', $event, width, height)"
                  >✥</button>
                  <button
                    type="button"
                    class="absolute -bottom-3 -right-3 grid h-7 w-7 touch-none place-items-center rounded-full bg-primary-600 text-xs text-white shadow-sm"
                    aria-label="Textblockgröße ändern"
                    @pointerdown.prevent="startPointerDrag('resize', $event, width, height)"
                  >⇲</button>
                </div>

                <UiPanel
                  padding="sm"
                  class="absolute z-10 flex items-center gap-1 bg-white/95 text-xs dark:bg-gray-900/95"
        ...
        <div class="space-y-4">
          <UiPanel v-if="active" class="space-y-4">
            ...
            <UiField label="Text" for-id="text-editor-content">
              <textarea id="text-editor-content" :value="active.text" rows="5" maxlength="100000" class="ui-control w-full" @input="onPanelInput"></textarea>
            </UiField>
          ...
          <UiPanel v-if="editCount" class="space-y-2">
            ...
            <ul class="max-h-72 space-y-1 overflow-y-auto">
              <li v-for="[key, edit] in editEntries" :key="key" class="flex items-center gap-2 rounded-xl px-2 py-1.5 hover:bg-black/5 dark:hover:bg-white/10">
                <UiButton variant="ghost" size="sm" class="min-w-0 flex-1 justify-start" @click="openEdit(key)">
                  <span class="truncate">{{ edit.text.trim() ? edit.text.slice(0, 50) : '(Block wird geleert)' }}</span>
                  <span class="shrink-0 text-xs opacity-70">S. {{ edit.page }}</span>
                </UiButton>
...
/**
 * Textbearbeitung mit direkter Seitenvorschau und vollständig gleichwertiger
 * Tastaturalternative. Dokument-, Seiten- und Renderwechsel entwerten ältere
 * Textblockantworten; globale Pointerlistener werden bei jedem Ausgangspfad
 * entfernt.
 */
...
function validBox(value: unknown): Box | null {
  ...
  const x0 = clamp(Number(raw.x0)); const y0 = clamp(Number(raw.y0);); const x1 = clamp(Number(raw.x1)); const y1 = clamp(Number(raw.y1))
...
function restoreSession(payload: { name: string; blob: Blob; state: unknown }): void {
  documentGeneration += 1; blockController?.abort(); finishEdit(); cleanupDrag()
...
watch(file, (currentFile) => {
  ...
  documentGeneration += 1; blockController?.abort(); finishEdit(); cleanupDrag(); blocks.value = []; edits.value = new Map(); page.value = 1; warnings.value = 0; error.value = null
  if (!currentFile) { workingBlob.value = null; return }
  workingBlob.value = currentFile; workingName.value = currentFile.name
})
watch(page, () => { finishEdit(); blocks.value = []; error.value = null })
...
async function loadBlocks(renderedPage: number): Promise<void> {
  ...
  try {
    ...
    if (!controller.signal.aborted && requestId === blockRequest && generation === documentGeneration) error.value = caught instanceof Error ? caught.message : 'Textblöcke konnten nicht geladen werden.'
  } finally {
...
async function startEdit(index: number): Promise<void> {
  ...
  await nextTick(); if (!editorEl.value || !active.value) return; editorEl.value.textContent = active.value.text; editorEl.value.focus(); const range = document.createRange(); range.selectNodeContents(editorEl.value); range.collapse(false); const selection = window.getSelection(); selection?.removeAllRanges(); selection?.addRange(range)
}
function openEdit(key: string): void { const [pagePart, indexPart] = key.split(':'); const targetPage = Number(pagePart); if (!Number.isFinite(targetPage)) return; page.value = targetPage; void nextTick(() => startEdit(Number(indexPart))) }
...
function commitActive(): void { const current = active.value; if (!current) return; const original = originalForActive(); const unchanged = original && current.text === original.text && current.font_size === original.font_size && current.color === original.color && current.font === original.font_name && JSON.stringify(current.box) === JSON.stringify(original.bbox); if (unchanged) edits.value.delete(current.key); else edits.value.set(current.key, { page: current.page, bbox: { ...current.box }, text: current.text, font_size: current.font_size, color: current.color, font: current.font }); edits.value = new Map(edits.value) }
...
function setGeometry(key: 'x' | 'y' | 'width' | 'height', event: Event): void { if (!active.value) return; const value = Number((event.target as HTMLInputElement).value) / 100; if (!Number.isFinite(value)) return; const box = active.value.box; const width = box.x1 - box.x0; const height = box.y1 - box.y0; if (key === 'x') { box.x0 = clamp(value, 0, 1 - width); box.x1 = box.x0 + width } else if (key === 'y') { box.y0 = clamp(value, 0, 1 - height); box.y1 = box.y0 + height } else if (key === 'width') box.x1 = clamp(box.x0 + Math.max(.01, value), box.x0 + .01, 1); else box.y1 = clamp(box.y0 + Math.max(.01, value), box.y0 + .01, 1); commitActive() }
...
function startPointerDrag(mode: 'move' | 'resize', event: PointerEvent, width: number, height: number): void {
  ...
  const move = (pointerEvent: PointerEvent) => { if (!active.value) return; const dx = (pointerEvent.clientX - startX) / Math.max(1, width); const dy = (pointerEvent.clientY - startY) / Math.max(1, height); if (mode === 'move') { const boxWidth = start.x1 - start.x0; const boxHeight = start.y1 - start.y0; const x0 = clamp(start.x0 + dx, 0, 1 - boxWidth); const y0 = clamp(start.y0 + dy, 0, 1 - boxHeight); active.value.box = { x0, y0, x1: x0 + boxWidth, y1: y0 + boxHeight } } else active.value.box = { ...start, x1: clamp(start.x1 + dx, start.x0 + .02, 1), y1: clamp(start.y1 + dy, start.y0 + .01, 1) } }
...
async function applyEdits(): Promise<void> { const blob = workingBlob.value; if (!blob || !edits.value.size || applying.value) return; finishEdit(); const generation = documentGeneration; applying.value = true; error.value = null; try { const formData = new FormData(); formData.append('file', blob, workingName.value); formData.append('edits', JSON.stringify([...edits.value.values()])); const response = await apiPost('/api/pdf-extras/edit-text-blocks', formData); const newBlob = await response.blob(); if (generation !== documentGeneration || workingBlob.value !== blob) return; warnings.value = Number(response.headers.get('X-Warnings') || 0); workingBlob.value = newBlob; blocks.value = []; edits.value = new Map(); active.value = null } catch (caught: unknown) { if (generation === documentGeneration) error.value = caught instanceof Error ? caught.message : 'Anwenden fehlgeschlagen.' } finally { if (generation === documentGeneration) applying.value = false } }
async function download(): Promise<void> { if (!workingBlob.value) return; const name = workingName.value.replace(/\.pdf$/i, '_bearbeitet.pdf'); await downloadBlob(new Response(workingBlob.value), name) }
function reset(): void { documentGeneration += 1; blockRequest += 1; blockController?.abort(); cleanupDrag(); file.value = null; workingBlob.value = null; blocks.value = []; edits.value = new Map(); active.value = null; warnings.value = 0; page.value = 1; pageCount.value = 1; error.value = null }
